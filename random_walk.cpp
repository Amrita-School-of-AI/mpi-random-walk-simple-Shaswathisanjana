...
void walker_process()
{
    ...
    int completed = 1;
    MPI_Send(&completed, 1, MPI_INT, 0, 0, MPI_COMM_WORLD);

    //    c. Break the loop. (Already handled in the while loop)
}

void controller_process()
{
    // 1. Determine the number of walkers (world_size - 1).
    int no_walkers = world_size - 1;
    int msg;
    MPI_Status status;

    // 2. Loop that many times to receive a message from each walker.
    for (int i = 0; i < no_walkers; i++)
    {
    ...
    std::cout << "Controller: All " << no_walkers << " walkers have finished." << std::endl;
}
